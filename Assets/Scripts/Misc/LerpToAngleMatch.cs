...
public class LerpToAngleMatch : MonoBehaviour
{
    ...
    void Update()
    {
        if (isActive)
        {
            Quaternion slerped = Quaternion.Slerp(myTransform.rotation, match.rotation * angleOffset, slerpRate);
            ...
            if (matchYOnly) // mostly only move on the y axis
            {
                ...
                Quaternion reslerped = Quaternion.Slerp(myTransform.rotation, dampenedMatch, slerpRate);
